...
import { X, ChevronLeft, ChevronRight, CheckCircle, XCircle, Calendar, MapPin, Phone, Upload, ImagePlus } from "lucide-react";
...
// Returns true if date+hour is less than 24h from now
const isTooSoon = (dateStr, slotHour) => {
  const booking = new Date(`${dateStr}T${String(slotHour).padStart(2, "0")}:00:00`);
  return booking.getTime() - Date.now() < 24 * 60 * 60 * 1000;
};
...
.rd-card{background:#fff;border-radius:18px;width:100%;max-width:520px;max-height:92vh;overflow:hidden;animation:rdFadeIn .22s ease both;position:relative;box-shadow:0 32px 80px rgba(0,0,0,.22);display:flex;flex-direction:column;}
.rd-step-content{animation:rdSlideIn .2s ease both;flex:1;overflow-y:auto;min-height:0;}
.rd-upload-zone{border:2px dashed #e2e8f0;border-radius:10px;padding:18px 14px;text-align:center;cursor:pointer;background:#f8fafc;transition:border-color .18s,background .18s;}
.rd-upload-zone:hover{border-color:#06b6d4;background:#f0fdfe;}
...
export default function ReservationDialog({ worker, user, initialProfession, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    reservationApi.getWorkerMonthAvailability(worker._id, profession)
      .then((data) => {
        ...
        const first = days.find((d) => d.slots?.some((s) => s.status === "available" && !isTooSoon(d.date, s.hour)));
        if (first) {
          setDate(first.date);
  ...
  const validateStep = useCallback((s) => {
    ...
    if (s === 1) {
      ...
      else if (isTooSoon(date, hour)) errs.hour = "Réservation impossible — il faut au moins 24h à l'avance";
    }
    if (s === 2) {
  ...
  const checkVideoDuration = (file) => new Promise((resolve) => {
    ...
    video.onloadedmetadata = () => { URL.revokeObjectURL(video.src); resolve(video.duration <= 300); };
  ...
  const handleMediaChange = async (e) => {
    ...
    const checks = await Promise.all(valid.map(checkVideoDuration));
    const tooLong = valid.filter((_, i) => !checks[i]);
    const okFiles = valid.filter((_, i) => checks[i]);

    if (tooLong.length) {
      setErrors((prev) => ({ ...prev, media: `Vidéo trop longue (max 5 min) : ${tooLong.map((f) => f.name).join(", ")}` }));
    } else {
      setErrors((prev) => { const n = { ...prev }; delete n.media; return n; });
    }

    if (okFiles.length) {
      setMediaFiles((prev) => [...prev, ...okFiles].slice(0, 5));
    }
    e.target.value = "";
  };
    ...
    <>
      ...
      <div className="rd-overlay" onClick={(e) => { if (e.target === e.currentTarget && !submitting && !result) onClose(); }}>
        <div className="rd-card">
          ...
          {/* Result overlay */}
          {result ? (
            <div style={{ padding:"40px 28px",textAlign:"center",overflowY:"auto",flex:1 }}>
              {result.ok ? (
                <>
                ...
                <>
                  ...
                  <div style={{ fontSize:20,fontWeight:800,color:"#0f172e",marginBottom:10 }}>Réservation impossible</div>
                  <div style={{ fontSize:13,color:"#64748b",lineHeight:1.6,marginBottom:28 }}>{result.message}</div>
                  <div style={{ display:"flex",gap:10,justifyContent:"center" }}>
                    <button onClick={() => { setResult(null); setStep(2); }} style={{ background:"#0f172e",color:"#fff",border:"none",borderRadius:10,padding:"12px 24px",fontSize:13,fontWeight:700,cursor:"pointer" }}>Modifier l'adresse</button>
                      ...
                      {avLoading ? (
                        ...
                        <div style={{ display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:7 }}>
                          {visibleDates.map((day) => {
                            const free  = (day.slots||[]).filter((s) => s.status === "available" && !isTooSoon(day.date, s.hour)).length;
                ...
                {step === 3 && (
                  <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
                    ...
                    <div>
                      ...
                      <div className="rd-upload-zone" onClick={() => fileInputRef.current?.click()}>
                        <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:6 }}>
                          <div style={{ width:40,height:40,borderRadius:10,background:"rgba(6,182,212,.1)",display:"flex",alignItems:"center",justifyContent:"center" }}>
                            <ImagePlus size={20} color="#06b6d4" />
                          </div>
                          <div style={{ fontSize:13,fontWeight:600,color:"#0f172e" }}>Cliquez pour ajouter des fichiers</div>
                          <div style={{ fontSize:10,color:"#94a3b8" }}>JPG, PNG, MP4, WebM · max 5 fichiers · vidéo ≤ 5 min</div>
                        </div>
                      </div>
                      <input ref={fileInputRef} type="file" style={{ display:"none" }}
                        accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime"
                        multiple onChange={handleMediaChange} />
                      {errors.media && <div style={{ color:"#ef4444",fontSize:11,marginTop:4 }}>{errors.media}</div>}
                      {mediaFiles.length > 0 && (
                        <div style={{ marginTop:8,display:"grid",gap:6 }}>
                          {mediaFiles.map((file, idx) => (
                            <div key={`${file.name}-${idx}`} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:8,padding:"6px 10px",fontSize:11,color:"#334155" }}>
                              ...
                              <button type="button" onClick={() => removeMedia(idx)} style={{ border:"none",background:"transparent",color:"#ef4444",fontWeight:700,cursor:"pointer",fontSize:11 }}>Retirer</button>
                            </div>
                          ))}
                        </div>
                      )}
